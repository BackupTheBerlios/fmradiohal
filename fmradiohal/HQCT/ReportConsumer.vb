Option Strict On
Imports System

Imports System.Windows.Forms
Imports System.Text
Imports System.Threading
'Imports HQCTS.RDS
Imports System.Diagnostics


'using HQCTS.USBHIDIO;
Namespace HQCT
    Friend Delegate Sub NewMessage_Delegate(ByRef IncMessage As HQCT.stIncMessage)
    Friend Class ReportConsumer
        Public Event NewMessage As NewMessage_Delegate

        Public mCom As HQCT.Communication
        Private mDevice As Device
        'Private mSignalProcessorStatus As SignalProcessorStatus
        'Private mrdsStatus As RDS.RDSStatus
        'Private mTunerSettings As TunerSettings

        Private shouldStop As Boolean '


        Private mreportIndex As Integer = 0

        Public ReadOnly Property ReportIndex() As Integer
            Get
                Return mreportIndex
            End Get
        End Property

        'Public Sub New(ByRef com As Communication, ByRef device As Device, ByRef signalProcessorStatus As SignalProcessorStatus, ByRef rdsStatus As Status, ByRef tunerSettings As TunerSettings)
        Public Sub New(ByRef lcom As HQCT.Communication, ByRef ldevice As Device)
            Me.mCom = lcom
            Me.mDevice = ldevice
            'Me.mSignalProcessorStatus = lsignalProcessorStatus
            'Me.mrdsStatus = lrdsStatus
            'Me.mTunerSettings = lTunerSettings
        End Sub 'New


        Public Sub ConsumeReport()
            Dim i As Integer
            Dim buffer As Byte()
            Dim Flag As Boolean
            Dim IncMessage As HQCT.stIncMessage
            Dim DeviceStatus As HQCT.enHQCT_STATUS
            While Not shouldStop
                Flag = False
                'mCom.SomethingInBuffer.WaitOne()
                SyncLock mCom.m_reportlist
                    If mCom.m_reportlist.Counter > 0 Then
                        Flag = True
                        buffer = mCom.m_reportlist.Remove
                    End If
                End SyncLock

                If Flag = False Then
                    Thread.CurrentThread.Suspend()
                Else




                    'If Not (mCom Is Nothing) Then
                    '    buffer = mCom.ReadInputReport()
                    'Else
                    '    buffer = Nothing
                    'End If
                    'If Not (buffer Is Nothing) Then

                    mreportIndex += 1

                    SyncLock mDevice
                        DeviceStatus = mDevice.Status
                    End SyncLock




                    Select Case DeviceStatus
                        'Case enHQCT_STATUS.HQCT_STATUS_UNKNOWN
                        'DeviceStatus = enHQCT_STATUS.HQCT_STATUS_1MESSAGE
                        'If buffer(0) = &H54 And buffer(31) = &H54 Then
                        '    DeviceStatus = enHQCT_STATUS.HQCT_STATUS_INIT
                        'Else
                        '    Dim outputBuffer As Byte() = New Byte(mcHID.OUTPUT_BUFFER_SIZE) {}
                        '    outputBuffer(1) = 128
                        '    mCom.SomethingInBuffer.WaitOne()
                        '    mcHID.Write(Handle, outputBuffer)
                        'End If
                    Case DeviceStatus.HQCT_STATUS_INIT ', enHQCT_STATUS.HQCT_STATUS_1MESSAGE, enHQCT_STATUS.HQCT_STATUS_UNKNOWN
                            If buffer(0) = &H54 And buffer(31) = &H54 Then
                                DeviceStatus = DeviceStatus.HQCT_STATUS_EEPROM1
                            End If

                        Case DeviceStatus.HQCT_STATUS_EEPROM1
                            [Array].Copy(buffer, 0, mDevice.EEPROM, 0, 32)


                            ' Gather the DAAs and alignment frequencies  
                            For i = 0 To (Math.Min(7, CInt(buffer(&HA)))) - 1
                                'mDevice.AlignFreq(i) = 87500 + buffer((&HB + 2 * i)) * 100
                                'mDevice.DAAs(i) = buffer((&HC + 2 * i))
                            Next i

                            Dim offset As Integer
                            For offset = 0 To 30
                                mDevice.TunerSettings(offset) = buffer(offset)
                            Next offset

                            DeviceStatus = DeviceStatus.HQCT_STATUS_EEPROM2

                        Case DeviceStatus.HQCT_STATUS_EEPROM2
                            [Array].Copy(buffer, 0, mDevice.EEPROM, 32, 32)


                            Dim offset As Integer
                            For offset = 0 To 30
                                mDevice.TunerSettings((32 + offset)) = buffer(offset)
                            Next offset

                            ' Subtract 31 from addresses. 

                            DeviceStatus = DeviceStatus.HQCT_STATUS_EEPROM3

                        Case DeviceStatus.HQCT_STATUS_EEPROM3
                            [Array].Copy(buffer, 0, mDevice.EEPROM, 64, 32)


                            'mDevice.FmTrailer(2) = buffer((&H43 - &H3E)) '5  Subtract 62 from addresses 
                            'mDevice.FmTrailer(3) = CByte(&H80 Or (buffer(&H4A - &H3E) And &H7F)) '12
                            'mDevice.FmTrailer(4) = CByte(((buffer((&H4E - &H3E)) And &HF) << 4) Or (buffer((&H51 - &H3E)) And &HF)) '16  19 

                            'mDevice.AmTrailer(2) = buffer((&H48 - &H3E)) '10

                            Dim offset As Integer
                            For offset = 0 To 30
                                mDevice.TunerSettings((64 + offset)) = buffer(offset)
                            Next offset

                            DeviceStatus = mDevice.Status.HQCT_STATUS_NORMAL

                            Console.Write("HQCTS: AlignFr = {")
                            'For i = 0 To mDevice.AlignFreq.Length - 1
                            'Console.Write(String.Format("{0:d}, ", mDevice.AlignFreq(i)))
                            'Next i
                            'Console.Write("} " + ControlChars.Lf + "HQCTS: DAAs = {")
                            'For i = 0 To mDevice.DAAs.Length - 1
                            'Console.Write(String.Format("{0:d}, ", mDevice.DAAs(i)))
                            'Next i
                            Console.WriteLine("}")

                            'Case mDevice.Status.HQCT_STATUS_SEEKING_UP2
                            '    i = mDevice.Freq
                            '    If buffer(2) >= mDevice.MinLevel Or (mDevice.Band = enHQCT_BAND.HQCT_BAND_FM And i <= mDevice.Param And i > mDevice.Param - 100) Or (mDevice.Band = enHQCT_BAND.HQCT_BAND_AM And i <= mDevice.Param And i > mDevice.Param - 10) Then
                            '        mDevice.Status = enHQCT_STATUS.HQCT_STATUS_NORMAL
                            '    Else
                            '        mDevice.Status = enHQCT_STATUS.HQCT_STATUS_SEEKING_UP1
                            '        If mDevice.Band = enHQCT_BAND.HQCT_BAND_FM Then
                            '            i += 100
                            '        Else
                            '            i += 10
                            '        End If
                            '        If mDevice.Band = enHQCT_BAND.HQCT_BAND_FM And i > mDevice.AlignFreq(6) Then
                            '            mCom.rawTuneFrequency(mDevice.AlignFreq(0))
                            '        Else
                            '            If mDevice.Band = enHQCT_BAND.HQCT_BAND_AM And i > 1500 Then
                            '                mCom.rawTuneFrequency(500)
                            '            Else
                            '                mCom.rawTuneFrequency(i)
                            '            End If
                            '        End If
                            '    End If
                            'Case mDevice.Status.HQCT_STATUS_SEEKING_DN2
                            '    i = mDevice.Freq
                            '    If buffer(2) >= mDevice.MinLevel Or (mDevice.Band = enHQCT_BAND.HQCT_BAND_FM And i >= mDevice.Param And i < mDevice.Param + 100) Or (mDevice.Band = enHQCT_BAND.HQCT_BAND_AM And i >= mDevice.Param And i < mDevice.Param + 10) Then
                            '        mDevice.Status = enHQCT_STATUS.HQCT_STATUS_NORMAL
                            '    Else
                            '        mDevice.Status = enHQCT_STATUS.HQCT_STATUS_SEEKING_DN1
                            '        If mDevice.Band = enHQCT_BAND.HQCT_BAND_FM Then
                            '            i -= 100
                            '        Else
                            '            i -= 10
                            '        End If
                            '        If mDevice.Band = enHQCT_BAND.HQCT_BAND_FM And i < mDevice.AlignFreq(0) Then
                            '            mCom.rawTuneFrequency(mDevice.AlignFreq(6))
                            '        Else
                            '            If mDevice.Band = enHQCT_BAND.HQCT_BAND_AM And i < 500 Then
                            '                mCom.rawTuneFrequency(1500)
                            '            Else
                            '                mCom.rawTuneFrequency(i)
                            '            End If
                            '        End If
                            '    End If
                        Case enHQCT_STATUS.HQCT_STATUS_NORMAL
                            If buffer(31) <> &H54 And buffer(31) <> &H74 Then
                                IncMessage.Bytes = buffer

                                RaiseEvent NewMessage(IncMessage)

                                'mSignalProcessorStatus.decode(buffer)

                                'mrdsStatus.decode(buffer)
                                'If mSignalProcessorStatus.AfUpdateSample Then
                                '    Debug.WriteLine("AF Update sample recevied!")
                                '    mCom.rawTuneFrequency(mDevice.Param)
                                'End If

                            End If
                    End Select
                    SyncLock mDevice
                        mDevice.Status = DeviceStatus
                    End SyncLock
                End If
                'Else
                '    'Thread.Sleep(10)
                '    Thread.CurrentThread.Suspend()
                'End If
            End While
        End Sub 'ConsumeReport


        Public Sub RequestStop()
            shouldStop = True
        End Sub 'RequestStop
    End Class 'ReportConsumer
End Namespace 'HQCT
